...
class SharpEloquentAutoUpdaterService {
    ...
    private function valuateFileAttribute($instance, $attr, $file)
    {
        ...
        if($file && $file != $instance->$attr)
        {
            ...
            $folderPath = $this->sharpRepository->getFileUploadPath($instance, $attr);
            sharp_move_tmp_file($file, $folderPath);

            // Then, update database
            $this->sharpRepository->updateFileUpload($instance, $attr, $file);
        }

        elseif(!$file && $instance->$attr)
        {
            // Delete
            $this->sharpRepository->deleteFileUpload($instance, $attr);
        }
    }
